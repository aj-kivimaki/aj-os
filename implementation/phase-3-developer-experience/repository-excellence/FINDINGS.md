...
| F-027 | M | **Blocking** | No | **40 errors are hidden by F-026** — real strictness violations under the repo's own `noUncheckedIndexedAccess` / `exactOptionalPropertyTypes` (`tsconfig.json:22-23`). **All 40 are in `tests/`; zero in `src/`.** By code: 24× TS2532 (possibly undefined), 7× TS2741, 3× TS2345, 3× TS18048, 2× **TS2352** (readonly→mutable casts tsc flags as *"may be a mistake"* — **in a repo whose immutability is contract-level**), 1× TS2379. Precedent: this gotcha hid **3 real defects** during SPEC-003. — ⚠️ **EVIDENCE CORRECTED at the M2 Planning Re-read (2026-07-17): this row previously said 46.** The 46 came from ad-hoc flags (`--target es2022` without the matching `lib`), inventing six false errors in `src/platform/retrieval/RetrievalService.ts` because `toSorted` is es2023; the real `tsconfig` sets `target: esnext`. **The original exploration said 40 — the author introduced the error while "verifying" it.** Below the FPCP threshold (no scope, sequencing, or acceptance change); identified, explained, recorded. | probe: `tsconfig` extending the real one with `rootDir: "."` + `include: ["src","tests"]` → **40**. ⚠️ **`rootDir` must be widened** — inheriting `./src` yields 58× TS6059 and never typechecks. |
...
| F-031 | M | Major | No | **Six strictness flags sit commented out** at `tsconfig.json:26-32`: `noImplicitReturns`, `noImplicitOverride`, `noUnusedLocals`, `noUnusedParameters`, `noFallthroughCasesInSwitch`, `noPropertyAccessFromIndexSignature`. Free wins left on the table. — ⚠️ **RISK CORRECTED at the M2 Planning Re-read (2026-07-17).** The plan warned these *"may fail on the two orphaned identity resolvers"* (F-042) and thus couple M2 to M3-A. **Verified false — exported symbols are not unused locals.** Actual damage: **one** unused type, `ProviderMetadata`. **F-031 has no coupling to M3-A**; a cross-milestone dependency the plan assumed does not exist. | `npx tsc --noEmit --noUnusedLocals --noUnusedParameters -p tsconfig.json` → **1 error**: `src/context-builder/providers/schema.ts(22,3) TS6196` |
