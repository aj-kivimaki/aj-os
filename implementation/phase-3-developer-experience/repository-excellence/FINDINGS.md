...
| F-064 | **J** | Minor | No | ✅ **CLOSED — "keep, justified" result** (REX-503; REX-D4, recorded as a result). **Measured, not assumed:** the genuinely-shareable helper is **already consolidated** — `stubGenerator` is exported from `tests/end-of-session/session-fixtures.ts` and imported by the suites that want it; the remaining copies are **suite-tuned with different signatures** (`stubGenerator` text-returning vs throwing vs no-arg; `stubGitPort` two shapes; even `makeProvider`'s three copies differ — `collection.test.ts`'s `provide()` **throws to assert non-execution**). The two identical `makeProvider`s are small **contract fixtures**, which the `support.ts` convention — **REX-D4's governing boundary** — deliberately keeps inline. Consolidating them would violate that convention for a negligible maintenance saving. **No change; recorded as a result.** — *(orig assumed byte-identical duplication.)* | the stub definitions; `session-fixtures.ts:164` |
| F-065 | M | Minor | No | ✅ **CLOSED** (REX-503) — `wiki.ts`'s `printReport` split into a pure **`formatWikiReport(report): string[]`** (exported, testable without stdout capture) + a thin `console.log` printer, following `session.ts`'s `formatSessionReport` exemplar. New `tests/cli/wiki-report.test.ts` (2 tests) asserts the report content directly; output identical (leading/trailing blank lines preserved). — *(orig: `console.log`s directly, untestable.)* | `wiki-report.test.ts` 2 green |
